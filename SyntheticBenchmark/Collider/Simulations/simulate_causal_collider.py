...
def set_graph_attributes(G, model_params):
    ...
    for node in G.nodes():
        ...
        if i < float(num_cols) / (num_celltypes - 1): # Cell type A is on the left quadrant


            if j >= num_rows / 2: # Cell type A
        ...
        elif ( (i >= float(num_cols) / (num_celltypes - 1) )&(i < 2.0 * float(num_cols) / (num_celltypes - 1)) ): # Cell type C is to the right of A and B
        ...
        else: # Cell type D is on the right third
...
def set_ode_parameters(G, initial_states, domain_height, domain_width, bws_ligand, bws_receptor, 
    ...
    for k in range(num_species):
        ...
        if k == 0:
            lig_x = (domain_width / (num_celltypes - 1)) * lig_x
            lig_y = 0.5 * domain_height * (1 + lig_y)

            rec_x = (domain_width / (num_celltypes - 1)) * (1.0 + rec_x)

        elif k == 1:

            lig_x = (domain_width / (num_celltypes - 1)) * lig_x
            lig_y = 0.5 * domain_height * lig_y
            rec_x = (domain_width / (num_celltypes - 1)) * (1.0 + rec_x)

        else: # K = 2

            lig_x = (domain_width / (num_celltypes - 1)) * (1.0 + lig_x)
            lig_y = domain_height * lig_y
            rec_x = (domain_width / (num_celltypes - 1)) * (2.0 + rec_x)
        ...
        for node in G.nodes():
            ...
            if k == 0: # For LR 1, cell type A is initialised with ligand 1, cell type B is initialised with receptor 2
                ...
                if celltype != 2: # Only cell type B has non-zero receptor
                ...
                if celltype != 2: # Only cell type C has non-zero receptor
                    receptor = 0.0 # Set receptor to be zero
                ...
                if celltype != 3: # Only cell type D has non-zero receptor
                    receptor = 0.0 # Set receptor to be zero
...
### Set-up for analysis
simulation_path = '../output/collider/'
...
causal_prods[0, 2] = 0.1 # LR complex 1 triggers L3
causal_prods[1, 2] = 0.1 # LR complex 2 triggers L3

celltype_causality = np.zeros((num_species, num_celltypes), dtype=np.float64)
celltype_causality[0, 2] = 1.0 # LR complex 1 triggers cell type 3
celltype_causality[1, 2] = 1.0 # LR complex 2 triggers cell type 3
...
blobwidths_ligand = [5.0, 10.0]
blobwidths_receptor = [20.0, 40.0]

### Define the ODE function
def collider_rhs(t, Y, dY, D, causal_prods, celltype_causality):
    ...
    for k in range(num_species):
        ...
        for index in range(num_nodes):
            ...
            causal_prod_term = sum(np.array([causal_prods[m, k]*celltype_causality[m, celltype]*Y[3*(m*num_nodes + index) + 2] for m in range(num_species)]))
...
# JIT the ODE RHS
rhs_cfunc = create_jit_rhs(collider_rhs, args_dtype)
...
for seed in range(num_realisations):
    ...
    # Save the ODE solutions
    np.savetxt(observational_path + "/collider_causal_network_obs_sol_" + str(seed) + ".csv", obs_sol_output, delimiter=',', header="t,x,y,type,L1,R1,LR1,L2,R2,LR2,L3,R3,LR3", comments="")
    ...
    # intervened_causal_prods[1, 2] += np.random.normal(loc=0.0, scale=intervention_scale)
    intervened_causal_prods[0, 2] += np.random.lognormal(mean=0.0, sigma=intervention_scale)
    intervened_causal_prods[1, 2] += np.random.lognormal(mean=0.0, sigma=intervention_scale)
    ...
    # Save the interventional solution
    np.savetxt(interventional_path + "/collider_causal_network_int_sol_" + str(seed) + ".csv", int_sol_output, delimiter=',', header="t,x,y,type,L1,R1,LR1,L2,R2,LR2,L3,R3,LR3", comments="")
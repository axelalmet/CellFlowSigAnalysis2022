...
collider_ad_object <- ad$read_h5ad("../adata_collider.h5ad")

collider.data.input <- t(py_to_r(collider_ad_object$X))
rownames(collider.data.input) <- as.factor(rownames(py_to_r(collider_ad_object$var)))
colnames(collider.data.input) <- rownames(py_to_r(collider_ad_object$obs))

# lee.data.input <- lee.data.input - min(lee.data.input) # Need to make sure the data isn't negative (it is in this case)
# access meta data
meta.data <- py_to_r(collider_ad_object$obs)
collider.identity <- data.frame(group = meta.data$type, row.names = row.names(meta.data))
...
obs.data.input <- collider.data.input[, obs.use]
int.data.input <- collider.data.input[, int.use]
...
write.csv(obs.net, file = "../Output/collider_communications_observation.csv")
...
write.csv(int.net, file = "../Output/collider_communications_intervention.csv")

...
def set_graph_attributes(G, model_params):
    ...
    for node in G.nodes():
        ...
        if i < float(num_cols) / num_celltypes: # Cell type A is on the left quadrant
        ...
        elif ( (i >= float(num_cols) / num_celltypes)&(i < 2.0 * float(num_cols) / num_celltypes) ): # Cell type B is to the left of the middle
        ...
        elif ( (i >= 2.0 * float(num_cols) / num_celltypes)&(i < 3.0 * float(num_cols) / num_celltypes) ): # Cell type C is to the right of the middle
        ...
        else: # Cell type D is on the right quadrant
    ...
    for k in range(num_species):
        ...
        for node in G.nodes():
            ...
            if k == 0: # For LR 1, cell type A is initialised with ligand 1, cell type B is initialised with receptor 2
                ...
                if celltype != 1: # Only cell type B has non-zero receptor
...
def chain_rhs(t, Y, dY, D, causal_prods, celltype_causality):
    ...
    for k in range(num_species):
        ...
        for index in range(num_nodes):
            ...
            ligand_flux = np.sum(np.array([D[index, n]*Y[3*(k*num_nodes + n)] for n in range(num_nodes)]))

            causal_prod_term = np.sum(np.array([causal_prods[m, k]*celltype_causality[m, celltype]*Y[3*m*num_nodes + 3*index + 2] for m in range(num_species)]))
...
# JIT the ODE RHS
rhs_cfunc = create_jit_rhs(chain_rhs, args_dtype)
...
for seed in range(num_realisations):
    ...
    np.savetxt(observational_path + "/chain_causal_network_obs_sol_" + str(seed) + ".csv", obs_sol_output, delimiter=',', header="t,x,y,type,L1,R1,LR1,L2,R2,LR2,L3,R3,LR3", comments="")
    ...
    np.savetxt(interventional_path + "/chain_causal_network_int_sol_" + str(seed) + ".csv", int_sol_output, delimiter=',', header="t,x,y,type,L1,R1,LR1,L2,R2,LR2,L3,R3,LR3", comments="")

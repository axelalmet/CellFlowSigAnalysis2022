...
### Chain ###
chain_ad_object <- ad$read_h5ad("../adata_chain.h5ad")

chain.data.input <- t(py_to_r(chain_ad_object$X))
rownames(chain.data.input) <- as.factor(rownames(py_to_r(chain_ad_object$var)))
colnames(chain.data.input) <- rownames(py_to_r(chain_ad_object$obs))

# lee.data.input <- lee.data.input - min(lee.data.input) # Need to make sure the data isn't negative (it is in this case)
# access meta data
meta.data <- py_to_r(chain_ad_object$obs)
chain.identity <- data.frame(group = meta.data$type, row.names = row.names(meta.data))
...
obs.data.input <- chain.data.input[, obs.use]
int.data.input <- chain.data.input[, int.use]
...
write.csv(obs.net, file = "../Output/chain_communications_observation.csv")
...
write.csv(int.net, file = "../Output/chain_communications_intervention.csv")

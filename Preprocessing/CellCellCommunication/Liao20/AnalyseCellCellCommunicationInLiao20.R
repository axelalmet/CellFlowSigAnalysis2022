...
liao.data.input <- t(py_to_r(liao_ad_object$X))
rownames(liao.data.input) <- rownames(py_to_r(liao_ad_object$var))
colnames(liao.data.input) <- rownames(py_to_r(liao_ad_object$obs))

# lee.data.input <- lee.data.input - min(lee.data.input) # Need to make sure the data isn't negative (it is in this case)
# access meta data
meta.data <- py_to_r(liao_ad_object$obs)
liao.identity <- data.frame(group = meta.data$celltype, row.names = row.names(meta.data))

healthy.use <- rownames(meta.data)[meta.data$group == "HC"] # extract the cell names from disease data
moderate.use <- rownames(meta.data)[meta.data$group == "M"] # extract the cell names from disease data
severe.use <- rownames(meta.data)[meta.data$group == "S"] # extract the cell names from disease data
...
healthy.meta <- meta.data[meta.data$group == "HC", ]
moderate.meta <- meta.data[meta.data$group == "M", ]
severe.meta <- meta.data[meta.data$group == "S", ]
...
write.csv(healthy.net, file = "/Users/axelalmet/Documents/scRNASeqAnalysisAndModelling/CellChat/Output/Liao2020/liao20_sub_communications_HC.csv")
write.csv(moderate.net, file = "/Users/axelalmet/Documents/scRNASeqAnalysisAndModelling/CellChat/Output/Liao2020/liao20_sub_communications_M.csv")
write.csv(severe.net, file = "/Users/axelalmet/Documents/scRNASeqAnalysisAndModelling/CellChat/Output/Liao2020/liao20_sub_communications_S.csv")
